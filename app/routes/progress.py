...
async def record_progress(
    ...
    db_document = (
        ...
        .filter(StudyDocument.id == request.document_id, StudyDocument.user_id == user.id)
        .first()
    )
    if not db_document:
        raise HTTPException(
    ...
    db_question = (
        ...
        .filter(DBMCQQuestion.id == request.question_id, DBMCQQuestion.document_id == request.document_id)
        .first()
    )
    if not db_question:
        raise HTTPException(
...
async def get_document_progress(
    ...
    total_questions = (
        db.query(DBMCQQuestion)
        .filter(DBMCQQuestion.document_id == document_id)
        .count()
    )
    ...
    questions_attempted = (
        ...
        .filter(UserProgress.user_id == user.id, UserProgress.document_id == document_id)
        .scalar()
    )
    ...
    correct_count = (
        ...
        .join(latest_attempts_subquery, UserProgress.id == latest_attempts_subquery.c.latest_id)
        .filter(UserProgress.is_correct == True)
        .scalar()
    )
    
    incorrect_count = questions_attempted - correct_count if questions_attempted else 0
    accuracy = (correct_count / questions_attempted * 100) if questions_attempted > 0 else 0.0
...
async def get_overall_stats(
    ...
    documents_with_progress = (
        db.query(StudyDocument)
        .filter(StudyDocument.user_id == user.id)
        .all()
    )
...
async def get_question_history(
    ...
    progress_records = (
        ...
        .filter(UserProgress.user_id == user.id, UserProgress.question_id == question_id)
